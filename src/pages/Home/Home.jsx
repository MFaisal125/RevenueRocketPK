...
export const Home = () => {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      // Optimized resize handler - only runs on significant size changes
      let lastWidth = window.innerWidth;
      let resizeTimeout = null;

      const handleResize = () => {
        // Only recalculate if width changed significantly (by 15% or more)
        const currentWidth = window.innerWidth;
        const percentChange = Math.abs(currentWidth - lastWidth) / lastWidth;
  ...
  // Intersection Observer for scroll animations - only if high-end device and not reduced motion
  // Remove this entire useEffect block
  // useEffect(() => {
  //   if (!isLoaded || prefersReducedMotion || deviceTier !== "high") return;
  //
  ...
  //
  //   // Observe only essential animatable elements
  //   document.querySelectorAll('.animate-on-scroll').forEach(el => {
  //     observer.observe(el);
  //   });
  //
  //   return () => observer.disconnect();
  // }, [isLoaded, prefersReducedMotion, deviceTier]);

  // Determine if we should show certain UI elements based on device tier
  const showEnhancedUI = deviceTier !== "minimal";
  const showFullUI = deviceTier === "high";
  const showAnimations = !prefersReducedMotion && deviceTier !== "minimal";
    ...
    <div className="ultra-premium-home">
      ...
      <style jsx>{`
        ...
        .hero-badge {
          ...
          position: relative;
          border: 1px solid rgba(255, 255, 255, 0.2);
          /* Remove opacity and transform properties */
          opacity: 1;
          transform: none;
          /* Remove transition */
        }
